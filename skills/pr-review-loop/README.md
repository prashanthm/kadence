...
- **Claude:** `claude auth login` (stored session) or `claude setup-token` — the only backend
...
`install` checks CLI login, writes the operator config overlay, installs a scheduler **every 15 min**, and runs one smoke firing. On macOS it installs launchd at `:00/:15/:30/:45`; on Windows it creates a Task Scheduler job named `kadence-pr-review-loop` that runs every 15 minutes from install time through Git Bash (firings may not align to quarter-hour marks the way launchd does). A firing still in progress when the next tick arrives is **skipped** (single-instance lock at `~/.local/share/kadence/pr-review-loop.lock`).
...
| Path | Purpose |
|------|---------|
| `~/.config/kadence/pr-review-loop.yaml` | Operator overlay (machine-specific keys) |
| `~/.local/share/kadence/pr-review-loop-firings.log` | JSONL firing history |
| `~/.local/share/kadence/pr-review-loop-latest.md` | Last firing summary |
...
python3 scripts/discover_pr_review_candidates.py \
  --config ~/.config/kadence/pr-review-loop.yaml --json
```
