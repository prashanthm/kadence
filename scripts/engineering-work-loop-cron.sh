#!/usr/bin/env bash
# Cron wrapper: preflight → agent → status report.
set -euo pipefail
...
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
  echo "skip: previous engineering-work-loop firing still in progress ($LOCK_DIR)" >&2
  exit 0
fi
echo "$$" > "$LOCK_DIR/pid"
trap 'rm -rf "$LOCK_DIR"' EXIT

if [[ -z "${ENGINEERING_LOOP_CONFIG:-}" ]]; then
  ENGINEERING_LOOP_CONFIG="$(SCRIPT_DIR_PY="$SCRIPT_DIR" "${PYTHON_CMD[@]}" -c "
import os, sys
sys.path.insert(0, os.environ['SCRIPT_DIR_PY'])
from engineering_work_loop_config import resolve_config_path
print(resolve_config_path())
")"
  export ENGINEERING_LOOP_CONFIG
fi
...
exec "${PYTHON_CMD[@]}" "$SCRIPT_DIR/engineering_work_loop_cron.py" --config "$ENGINEERING_LOOP_CONFIG" --json
